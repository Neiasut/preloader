import {
  PreloaderDomConstructorMessageError,
  PreloaderDomDestructorMessageError
} from './interfaces';
import PreloaderDom from './PreloaderDom';

export const defaultConstructorMessageError: PreloaderDomConstructorMessageError = (
  instance: PreloaderDom
): Element => {
  const element = document.createElement('div');
  element.innerHTML =
    '<div>Server connection error!</div><div><button>OK</button></div>';
  const clickClose = (): void => {
    instance.hide();
  };
  const button = element.querySelector('button');
  button.addEventListener('click', clickClose);
  instance.saveListener('clickClose', 'click', button, clickClose);
  return element;
};

export const defaultDestructorMessageError: PreloaderDomDestructorMessageError = (
  instance: PreloaderDom
): void => {
  instance.removeListener('clickClose');
};

export const constructorCanvas = (imgUrl: string): Element => {
  const element = document.createElement('div');
  element.classList.add('preloader-canvas');
  element.innerHTML = `
<div class="preloader-background"></div>
<div class="preloader-window">
    <div class="preloader-img">
        <img src="${imgUrl}" alt="load data"/>
    </div>
    <div class="preloader-error"></div>
</div>
`;
  return element;
};

const preloaderDomStyles = (): string => `
.preloader {
  min-width: 80px;
  min-height: 80px;
  position: relative;
...
.preloader-canvas, .preloader-canvas * {
...
.preloader-window {
  ...
  max-width: 100%;
  background: white;
  box-shadow: 0 10px 15px 0 rgba(18, 41, 62, 0.1);
  width: 80px;
  min-height: 80px;
  display: flex;
  flex-direction: column;
  justify-content: center;
  border: 1px solid rgba(18, 41, 62, 0.2);
}
.preloader-img{
  text-align: center;
}
...
.preloader-show .preloader-canvas {
  visibility: visible;
}
.preloader-error{
  display: none;
